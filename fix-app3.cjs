...
let funcIndex = lines.findIndex(l => l.includes('function showPersonDetails('));
if (funcIndex !== -1) {
  let startIndex = lines.findIndex((l, i) => i > funcIndex && l.includes('try {'));
  if (startIndex !== -1 && lines[startIndex+1].includes("const role = directedMovies.length > 0")) {
    ...
    console.log('Could not find try { with expected contents.');
  }
} else {
  console.log('Could not find showPersonDetails');
}
